...
export class SportsDbClient {
  ...
  async getDailyEvents(date: string, leagueName?: string): Promise<SportsDbEvent[]> {
    const data = await this.get<{ events?: SportsDbEvent[] }>('eventsday.php', leagueName ? { d: date, l: leagueName } : { d: date });
    return data.events ?? [];
  }
